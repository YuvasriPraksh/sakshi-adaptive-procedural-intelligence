...
export default function GraphPage() {
  ...
  const [casesList, setCasesList] = useState<InvestigationCase[]>(CASES);
  const [selected, setSelected] = useState<string>(caseId ?? CASES[0].id);

  // Fetch real cases from API on mount
  useEffect(() => {
    caseService.list({ pageSize: 50 }).then((res) => {
  ...
  const caseData = casesList.find((c) => c.id === selected) ?? casesList[0];
    ...
    <DashboardLayout>
      <div className="space-y-6 max-w-[1600px] mx-auto animate-in fade-in duration-500">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-border pb-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-2 max-w-2xl">
              Deterministic pre-trial dependency engine. Evaluates real-time procedural prerequisites, uncovers root blockers, and measures investigation readiness.
            </p>
          </div>
              ...
              >
                {casesList.map((c) => (
                  <option key={c.id} value={c.id}>
                    {c.caseNumber} — {c.victimCode} ({CRIME_LABEL[c.crimeType] || c.crimeType})
                  </option>
                ))}
              </select>
        ...
        <div className="flex flex-wrap items-center gap-4 rounded-xl border border-border bg-card px-5 py-3.5 shadow-sm">
          ...
          <span className="text-sm font-medium text-foreground">{CRIME_LABEL[caseData.crimeType] || caseData.crimeType}</span>
          <div className="w-1.5 h-1.5 rounded-full bg-border" />
          <span className="text-sm text-muted-foreground">{caseData.district}</span>
          <div className="w-1.5 h-1.5 rounded-full bg-border" />
          <StatusBadge variant={STATUS_BADGE[caseData.status] || "default"} size="sm" dot>
